			...
			editor.on( 'themeSpace', function( event )
				{
					...
					{
						event.data.html +=
							'<span id="'ement = CKEDITOR.document.getById( idBase + ( editor._.elementsPath.list.length - 1 ) );
							element.focus();
							return false;
			...
			editor.on( 'selectionChange', function( ev )
				{
					...
					while ( element )
					{
						...
						if ( !ignore )
						{
							...
							// HTML (because we are doing it in rever + spaceId + '_label" class="cke_voice_label">' + editor.lang.elementsPath.eleLabel + '</span>' +
