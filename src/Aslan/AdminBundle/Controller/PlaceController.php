...
class PlaceController extends StoreController
{

    public function placeImage($img, $val)
    {
        $blogsNew = array();
    ...
    public function adminPlaceFormAction(Request $request)
    {
        $session = $this->getRequest()->getSession();
        $user_id = $session->get('user_id');
        if (!isset($user_id)) {
            ...
            //$onemenu = parent::oneMenuAction($id);
            $id = is_array($request->get('names')) ? $request->get('names')['id'] : null;
            ...
            if ($request->getMethod() == 'POST') {
                $form->bindRequest($request);
                $src = parent::blogsUpdateAction($page);
                return new Response($src);
            } else {
                return $this
                    ->render(
    ...
    public function adminPlaceAction(Request $request)
    {
        $session = $this->getRequest()->getSession();
        $user_id = $session->get('user_id');
        if (!isset($user_id)) {
    ...
    public function idPlaceAction(Request $request)
    {
        $id = $request->request->get('id');
        $page = parent::oneBlogsAction($id);
        return new Response($page->getJson());
    }
