...
use Aslan\AdminBundle\Form\Type\AfishaType;
use Aslan\AdminBundle\Entity\Afisha;

class AfishaController extends StoreController {

    public function afishaImage($img, $val) {
        $blogsNew = array();
    ...
    public function adminImgAction(Request $request, $type) {
        ...
        if (!isset($user_id)) {
            ...
            $albums = parent::findAllAction('Afisha');
            $page = new Images();
    ...
    public function adminAfishaFormAction(Request $request) {
        $session = $this->getRequest()->getSession();
        $user_id = $session->get('user_id');
        if (!isset($user_id)) {
            return $this
            ...
            //$onemenu = parent::oneMenuAction($id);
            $page = new Afisha();
            $places = parent::blogsAllAction();
            $categorys = parent::categoryAllAction();
            $form = $this->createForm(new AfishaType($places, $categorys), $page);
            if ($request->getMethod() == 'POST') {
                $form->bindRequest($request);
                $file = $form['img']->getData();
                $name = $page->getName();
                ...
                if ($file) {
                    ...
                    $fileName = 'img' . date('YmdHis') . '.' . $extension;
                    $file->move('media/pictures/', $fileName);
                    $blogs = $this->afishaImage('media/pictures/' . $fileName, 250);
                    $this->get('image.handling')->open('media/pictures/' . $fileName)->resize($blogs['width'], $blogs['height'])->save('media/pictures/small' . $fileName);
                }
                $src = parent::afishaUpdateAction($name, $fileName, $id, $date, $place, $category, $text, $color);
                return new Response($src);
            } else {
                return $this
                                ->render(
                                        'AslanAdminBundle:Afisha:admin.blogsForm.html.twig', array("form" => $form->createView()));
            }
        }
    }

    public function adminAfishaAction(Request $request) {
        $session = $this->getRequest()->getSession();
        $user_id = $session->get('user_id');
        if (!isset($user_id)) {
            return $this
            ...
            //$onemenu = parent::oneMenuAction($id);
            $brandall = parent::afishaAllAction();
            
            return $this
                            ->render(
                                    'AslanAdminBundle:Afisha:admin.blogs.html.twig', array(
                                "secondmenuall" => $brandall,
                                'menu' => 9       
            ));
        }
    }
    ...
    public function adminRemoveAfishaAction(Request $request) {
        $id = $request->request->get('id');
        parent::afishaRemoveAction($id);
        return new Response();
    }
    ...
    public function idAfishaAction(Request $request) {
        $id = $request->request->get('id');
        $page = parent::oneAfishaAction($id);
        $ret = array();
        $ret[0][0] = $page->getId();
        $ret[0][1] = $page->getName();
        $ret[0][2] = $page->getPlace()->getId();
        $ret[0][3] = $page->getCategory()->getId();
        $ret[0][4] = $page->getText();
        $ret[0][5] = $page->getColor();
        return new Response(json_encode($ret));
    }
